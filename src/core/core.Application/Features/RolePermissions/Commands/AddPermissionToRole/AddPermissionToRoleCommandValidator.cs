    ...
    public sealed class AddPermissionToRoleCommandValidator : AbstractValidator<AddPermissionToRoleCommand>
    {
            ...
            IRolePermissionService rolePermissionService)
        {
            ...
            RuleFor(x => x)
                ...
                .WithMessage("Permission is already assigned to role.");
        }

        private async Task<bool> RoleExistsAsync(AddPermissionToRoleCommand command, CancellationToken ct)
        {
            var role = await _roleService.TryGetByIdAsync(command.RoleId, ct);
            return role is not null;
        }

        private async Task<bool> PermissionExistsAsync(AddPermissionToRoleCommand command, CancellationToken ct)
        {
            var permission = await _permissionService.TryGetByIdAsync(command.PermissionId, ct);
            return permission is not null;
        }

        private async Task<bool> NotDuplicateAsync(AddPermissionToRoleCommand command, CancellationToken ct)
        {
            bool assigned = await _rolePermissionService.IsPermissionAssignedToRoleAsync(command.RoleId, command.PermissionId, ct);
            return !assigned;
        }
    }
}
